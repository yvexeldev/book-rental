...
export class RentalService implements IRentalService {
    ...
    async create(createRentalDto: CreateRentalDto) {
        try {
            const rental = await this.prismaService.$transaction(
                async (prisma) => {
                    ...
                    if (!book) {
                        throw new NotFoundException(
                            this.i18nService.t('book.NOT_FOUND'),
                        );
                    }
        ...
        try {
            const rental = await this.prismaService.$transaction(
                async (prisma) => {
                    ...
                    if (!existingRental) {
                        throw new NotFoundException(
                            this.i18nService.t('rental.NOT_FOUND'),
                        );
                    }
                    ...
                    if (
                        ...
                        await prisma.book.update({
                            ...
                            data: {
                                available_copies:
                                    existingRental.book.available_copies + 1,
                            },
                        });
                        ...
                        await prisma.book.update({
                            where: { id: updateRentalDto.bookId },
                            data: {
                                available_copies: newBook.available_copies - 1,
                            },
                        });
                    }
                    ...
                    const updatedRental = await prisma.rental.update({
                        where: { id },
                        data: {
                            ...(updateRentalDto.userId && {
                                userId: updateRentalDto.userId,
                            }),
                            ...(updateRentalDto.bookId && {
                                bookId: updateRentalDto.bookId,
                            }),
